...
- 第 20 屆現行法 **38 份 / 768 條 / 約 9 萬字**，全部可抽字，已轉成結構化 JSON。
...
- 因為語料只有 **768 條**，兩個索引都是小事，**兩個都建**，查詢時做關鍵字 + 語意的混合排序。
...
輸入進來後，用四種訊號把 768 條收斂成「該對照的十幾條」，並附上命中理由：
...
### 先挑戰你的假設
你說「法規查詢是地基」。**部分同意，但更精確地說：真正的地基是「結構化法規資料層」，不是查詢功能本身。** 而那個資料層我今天已經做掉大半（768 條的 JSON）。查詢是「第一個該做的功能」，但不是三者的前提——三者共同的前提是資料層。
